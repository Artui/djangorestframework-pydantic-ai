...
class QueryParam:
    ...
    django-restql field selection (``?query=`` / ``?fields=``), a custom
    serializer that branches on the query string, or a ``filter_set``. Register
    them on :class:`~rest_framework_pydantic_ai.SpecToolset` toolset-wide
